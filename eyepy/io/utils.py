...
def _create_properties(version, specification_getter):
    """Dynamically create properties for different version of the .vol export"""
    fields = [entry[0] for entry in specification_getter(version)]
    fmts = [entry[1] for entry in specification_getter(version)]
    funcs = [entry[2] for entry in specification_getter(version)]
...
def _create_properties_xml(version, specification_getter):
    """Dynamically create properties for different version of the .xml export"""
    fields = [entry[0] for entry in specification_getter(version)]
    locs = [entry[1] for entry in specification_getter(version)]
    funcs = [entry[2] for entry in specification_getter(version)]
