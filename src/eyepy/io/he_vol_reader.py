...
bscan_format = cs.Struct(
    "version" / cs.PaddedString(12, "ascii") * "Version identifier",
    "bscan_hdr_size"
    / cs.Int32ul
    * "Size of the B-Scan header in bytes. It is identical to the same value of the file header.",
    "start_x" / cs.Float64l * "X-Coordinate of the B-Scan's start point in mm.",
    "start_y" / cs.Float64l * "Y-Coordinate of the B-Scan's start point in mm.",
    "end_x"
    / cs.Float64l
    ...
    * "Layer segmentations for 17 Layers. If layers are not segmented the data is empty",
    "__empty"
    / cs.Padding(cs.this._.bscan_hdr_size - 256 - 17 * 4 * cs.this._.size_x)
...
vol_format = cs.Struct(
    "version" / cs.PaddedString(12, "ascii") * "Version identifier: HSF-OCT-xxx",
    "size_x"
    / cs.Int32ul
    ...
    * "Number of samples in an A-Scan, i.e. B-Scan height in pixel",
    ...
    "field_size_slo" / cs.Int32ul * "Horizontal field size of the SLO image in dgr",
    "scan_focus" / cs.Float64l * "Scan focus in dpt",
    "scan_position"
    / cs.PaddedString(4, "ascii")
    ...
    * """Scan pattern type:
    ...
    "bscan_hdr_size" / cs.Int32ul * "Size of the Header preceding each B-Scan in bytes",
    "id"
    / cs.PaddedString(16, "ascii")
    ...
    * "Unique identifier of the reference OCT-scan (zero terminated string)."
    ...
    "pid" / cs.Int32ul * "Internal patient ID used by HEYEX.",
    "patient_id"
    / cs.PaddedString(24, "ascii")
    * "User-defined patient ID (zero terminated string).",
    "dob" / FloatDate * "Patient's date of birth",
    "vid" / cs.Int32ul * "Internal visit ID used by HEYEX.",
    "visit_id"
    / cs.PaddedString(24, "ascii")
