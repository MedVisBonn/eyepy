...
def test_grid_2_rings():
    output_OD = grid(
        mask_shape=(7, 7), radii=(1, 3), laterality='OD', n_sectors=1, rotation=0
    )
    output_OS = grid(
        mask_shape=(7, 7), radii=(1, 3), laterality='OS', n_sectors=1, rotation=0
    )
...
def test_grid_1_circle_with_4_sectors_with_offset_45():
    output_OD = grid(
        mask_shape=(8, 8), radii=(3,), laterality='OD', n_sectors=(4,), rotation=(45,)
    )
    output_OS = grid(
        mask_shape=(8, 8), radii=(3,), laterality='OS', n_sectors=(4,), rotation=(45,)
    )
